...
public:
    ...
    bool VerifyTypeInfo(const CharString& propName, const CharString& typeAlias) {
        if (this->HasDebugInfo()) {
            ...
            //#NOTE_SK: these archives are self-describing, so a mismatch means we are reading
            //          a schema we do not actually understand - a Metro 2033 Redux skeleton
            //          through the Exodus layout, say. Carrying on regardless used to walk the
            //          cursor off into hyperspace and take the process down with it, so latch
            //          the reader as bad instead and let the caller notice via Good().
            if (name != propName || type != typeAlias) {
                mBad = true;
                return false;
            }
        }
    ...
    MetroReflectionReader OpenSection(const CharString& sectionName, const bool nameUnknown = false) {
        if (this->HasNoSections()) {
            ...
            //#NOTE_SK: this used to dereference the cursor straight away, which reads past the
            //          end of the buffer the moment we are handed an archive whose schema we
            //          got wrong - and that is a segfault, not a parse failure.
            if (mBad || mStream.Remains() < (sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint8_t))) {
            ...
            if (nameUnknown || sectionNameCrc == crc) {
                ...
                // a section must at least carry its own flags byte, and must fit in what is left
                if (sectionSize < 1 || (sectionSize - 1) > mStream.Remains()) {
                    mBad = true;
                    return MetroReflectionReader();
                }
    ...
    void ReadStructArray(const CharString& memberName, MyArray<T>& v) {
        ...
        if (s.Good()) {
            ...
            //#NOTE_SK: never resize on an unvalidated count - a misread schema hands us
            //          something like 4 billion here and the allocation alone kills us.
            //          Every element costs at least a couple of bytes on the wire.
            if (!s.Good() || arraySize > (s.GetStream().Remains() / 2)) {
                this->CloseSection(s);
                return;
            }
...
private:
    MemStream           mStream;
    // set when a property name or type does not match what we expect - see VerifyTypeInfo
    bool                mBad = false;
