...
struct StringsTable {
    const char* GetString(const size_t idx) const {
        //#NOTE_SK: the index comes straight off the wire. If our idea of the schema is even
        //          slightly off we land here with garbage, and indexing blindly read past the
        //          end of the vector and took the process down.
        static const char* kEmpty = "";
        return (idx < this->strings.size()) ? this->strings[idx] : kEmpty;
    }
...
public:
    ...
    inline size_t Remains() const {
        //#NOTE_SK: the cursor must never sit past the end, but if it ever does this
        //          subtraction would wrap around into a gigantic number and every
        //          "is there enough data left?" check downstream would happily pass
        return (this->cursor < this->length) ? (this->length - this->cursor) : 0;
    }
    ...
    T ReadTyped() {
        ...
        if (this->cursor + sizeof(T) <= this->length) {
            ...
            //#NOTE_SK: on a truncated tail this used to return 0 and leave the cursor parked
            //          short of the end, so every `while (!stream.Ended())` reader in here
            //          would spin on it forever. Swallow the remainder instead, that way the
            //          stream reports Ended() and those loops all terminate.
            this->cursor = this->length;
        }
        return result;
    }
    ...
    CharString ReadStringZ() {
        ...
        //#NOTE_SK: an unterminated string runs the scan right up to the end, and stepping
        //          over the terminator that isn't there used to park the cursor one byte
        //          past the buffer. From there Ended() never came true, Remains() wrapped
        //          around and GetDataAtCursor() handed out an out-of-bounds pointer.
        this->cursor = std::min<size_t>(i + 1, this->length);
