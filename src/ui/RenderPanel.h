...
namespace MetroEX {
    ...
    private:
        // set when InitGraphics() bails out, so the UI can say what actually went wrong
        String^                     mInitError;
    ...
    private:
        ...
        // size the swap chain was last built for, so we can notice the panel outgrowing it
        int                         mBackBufferWidth;
        int                         mBackBufferHeight;
        // 1x1 opaque white, bound wherever a mesh has no texture of its own
        ID3D11Texture2D*            mWhiteTex;
