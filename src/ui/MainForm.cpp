...
namespace MetroEX {
    ...
    //#NOTE_SK: version of the archive that is currently open, 0 when there is none.
    //          Models parse for every supported game, but skeletons, motions and levels
    //          live in a different container in the Redux-era games, so those file kinds
    //          stay unclassified there and fall back to plain raw extraction.
    static size_t sArchiveVersion = 0;

    static const size_t kVFXVersionExodusAssets = 3;

    static FileType DetectFileType(const MetroFile& mf) {
        FileType result = FileType::Unknown;

        const bool exodusAssets = (sArchiveVersion >= kVFXVersionExodusAssets);
        ...
        if (name->EndsWith(L".dds") ||
            ...
            //#NOTE_SK: the "c" variants are Crunch streams - the bulk of the Redux textures
            name->EndsWith(L".512c") ||
            name->EndsWith(L".1024c") ||
            name->EndsWith(L".2048c")) {
            ...
            result = exodusAssets ? FileType::Motion : FileType::Unknown;
    ...
    void MainForm::MainForm_FormClosing(System::Object^, System::Windows::Forms::FormClosingEventArgs^) {
        //#NOTE_SK: a loaded archive puts ~82k items into the tree view, and the Win32 tree
        //          control deletes items far worse than linearly - measured on Metro 2033
        //          Redux, tearing that tree down inside Form.Close() takes over a minute,
        //          during which the window is already gone and the process just hangs about.
        //          Nothing here needs saving once the user has asked to quit: extraction
        //          writes its files as it goes and the log is flushed right below. So we
        //          close the log and let the OS reclaim everything in one go.
        //
        //          The proper cure is to stop filling the tree with every file up front and
        //          populate folders on expand instead - that is a bigger change to the file
        //          list and the search box, and it is noted in the readme.
        if (mExtractionThread != nullptr && mExtractionThread->IsAlive) {
            // an extraction is still writing files - let it wind down the normal way
            return;
        }

        LogPrint(LogLevel::Info, "shutting down");
        LogClose();
...
//#endif
        ...
        if (!mRenderPanel->InitGraphics()) {
            String^ details = mRenderPanel->GetInitError();
            this->ShowErrorMessage(String::Format(L"Failed to initialize Direct3D 11 graphics, the 3D viewer will be unavailable.\n\n{0}",
                                                  String::IsNullOrEmpty(details) ? L"No further details available." : details));
        }
    ...
    void MainForm::toolBtnFileOpen_Click(System::Object^, System::EventArgs^) {
        OpenFileDialog ofd;
        ofd.Title = L"Open Metro archive (content.vfx)...";
        ofd.Filter = L"Metro archives (*.vfx)|*.vfx|All files (*.*)|*.*";
        ofd.FilterIndex = 0;
        ...
        if (ofd.ShowDialog(this) == System::Windows::Forms::DialogResult::OK) {
            ...
            if (mVFXReader->LoadFromFile(StringToPath(ofd.FileName))) {
                // has to be set before anything calls DetectFileType
                sArchiveVersion = mVFXReader->GetVersion();
                ...
                // spell out which game we actually recognised, it saves a trip to the log
                this->Text = String::Format(L"MetroEX - {0} - {1}",
                                            marshal_as<String^>(mVFXReader->GetGameName().c_str()),
                                            marshal_as<String^>(mVFXReader->GetSelfName().c_str()));
            } else {
                delete mVFXReader;
                mVFXReader = nullptr;
                sArchiveVersion = 0;
                ...
                this->ShowErrorMessage(L"Unsupported or damaged archive.\n\nSupported: Metro 2033 Redux, Metro Last Light Redux, Arktika.1 and Metro Exodus .vfx files.\nSee log.txt next to the executable for details.");
            }
    ...
    void MainForm::TextureSaveHelper(const fs::path& folderPath, const FileExtractionCtx& ctx, const CharString& name) {
        const CharString textureName = CharString("content\textures\\") + name;

        //#NOTE_SK: best resolution first. The Redux games keep most of their textures as
        //          Crunch-compressed .512c/.1024c/.2048c which we cannot decode yet, so those
        //          are deliberately not in this list - only what MetroTexture can actually read.
        //#NOTE_SK: highest resolution first, and the Crunch variants alongside the plain
        //          ones - in the Redux archives the "c" files are usually the only ones there
        static const char* kExts[] = { ".2048", ".2048c", ".1024", ".1024c", ".512", ".512c", ".dds" };

        size_t textureIdx = MetroFile::InvalidFileIdx;
        for (const char* ext : kExts) {
            textureIdx = mVFXReader->FindFile(textureName + ext);
        ...
        if (!resultPath.empty()) {
            ...
            if (stream) {
                ...
                if (mdl.LoadFromData(stream, mVFXReader, ctx.fileIdx)) {
                    ...
                    //#NOTE_SK: the textures are written by the exporter itself now - it is the
                    //          only thing that knows which files the material ends up naming
