...
void DDS_DecompressBC1(const void* inputBlocks, void* outPixels, const size_t width, const size_t height) {
    ...
    //#NOTE_SK: BC1 carries no alpha of its own, but every caller here hands us a tightly
    //          packed RGBA buffer, same as BC2/BC3/BC7 do. Writing 3 bytes per pixel the
    //          way this used to smeared the image sideways and left the last quarter of
    //          the buffer untouched, so we go through it at the proper 4 byte stride and
    //          prime the alpha channel to opaque first.
...
size_t DDS_GetCompressedSizeBCn(const size_t width, const size_t height, const size_t numMips, const size_t bytesPerBlock) {
    size_t w = width;
    size_t h = height;
    size_t result = 0;
...
size_t DDS_GetCompressedSizeBC7(const size_t width, const size_t height, const size_t numMips) {
    return DDS_GetCompressedSizeBCn(width, height, numMips, 16);
}
...
void DDS_MakeDX9Header(DDSURFACEDESC2& desc, const size_t w, const size_t h, const size_t numMips, const uint32_t fourCC) {
    memset(&desc, 0, sizeof(desc));
